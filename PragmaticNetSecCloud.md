...
* *Security groups* - These should be used by default and set to deny by default. Only open them up the absolute minimum of access needed. Since cloud services allow you to right size resources far more easily than when you run things on your own hardware, we find most organizations tend to deploy a far fewer number of services on any individual instance, which directly translates to having to open up fewer network ports. A large number of cloud deployments we've evaluated use only a good base architecture and security groups for their network security.
* *ACLs* - These mostly make sense in hybrid deployments where you need to more closely match or restrict communications between the data center and the cloud. Security groups are usually a better choice and we only recommend falling back to ACLs or subnet-level firewalling when you can't hit your security objectives otherwise.
...
This is a simplified example of a public network on Azure. All the components run on Azure, with nothing in the enterprise data center, and no VPN connections. Management of all assets is over the Internet. Since we can't show all the pieces or configuration settings in the diagram, here are some specifics:
...
* Each instance is provided a public IP address and domain name, but you can't access anything that isn't opened up with a defined service endpoint. Think of the endpoint as port forwarding (because it pretty much is).
* The service endpoint can point to the load balancer, which, in turn is tied to the auto scale group. You set rules on instance health, performance, and availability and the load balancer and auto scale group will provision and deprovision servers as needed, plus handle the routing. The IP addresses of the instances will change as these updates take place.
* The *Network Security Groups (NSGs)* will restrict access to each instance. In Azure you can also apply them to subnets. In this case we would apply them on a per-server basis. Traffic would be restricted to only whatever services is being provided by the application, and would deliberately DENY traffic between instances on the same subnet. By default Azure allows this traffic, unlike Amazon.
* The NSG can also restrict traffic to the instances to only from the load balancer, disabling direct Internet access. Ideally you never need to log into the servers since they are in an auto scale group, so you can also disable all the management/administration ports.

There is more, but if you follow this pattern the result is a hardened server, with no administrative traffic, protected both with Azure's default protections and the security groups. For the record, on Azure you are often much better off using their PaaS versions of things like web servers, instead of manually building the infrastructure like this.

## Basic Private Network on Amazon Web Services

Amazon works a little differently than Azure (okay, a lot). This example is a Virtual Private Cloud (VPC, their name for a virtual network) that is completely private, without any Internet routing, connected to a data center through a VPN connection. 



* This shows a class B network with two smaller subnets. In AWS you would place each subnet in a different *Availability Zone* (what we called a zone) for resilience if one goes down (they are separate physical data centers). 
* You configure the VPN gateway through the AWS console or API, and then configure the client side of the VPN connection on your own hardware. Amazon maintains the VPN gateway, you don't directly touch it or maintain it, but you do need to maintain everything on your side of the connection (and it needs to be a hardware VPN).
* You adjust the routing table on your internal network to send all traffic for the 10.0.0.0/16 network over the VPN connection to AWS. This is why it's called a "virtual" private cloud. Instances can't see the Internet, but you do have that gateway that's Internet accessible. 
...
* Instances, by default, are protected with a Security Group that denies all inbound traffic, and allows all outbound traffic. Unlike in Azure, instances on the same subnet can't talk to each other. You can't connect to them through the corporate network, until you open them up. AWS Security Groups are *ALLOW rules only*. You can't explicitly deny traffic, only open up allowed traffic. In Azure you create Service Endpoints to explicitly route traffic, then use network security groups to allow or deny on top of that (and within the virtual network). AWS uses security groups for both functions - opening a security group allows traffic through the private IP (or public IP if it is public facing).
* Our example has no ACLs, but you *could* put an ACL in place to block the two subnets from talking to each other. ACLs in AWS are there by default, but allow all traffic. An ACL in AWS is *not stateful* so you have to create rules for all bidrectional traffic. ACLs in AWS work better as a DENY mechanism.
* A public network on AWS looks relatively similar to our Azure sample (which we deliberately designed to look similar). The key differences being how security groups and service endpoints function.

## Hybrid Cloud on Azure

This builds on the descriptions in our two previous examples. In this case the web servers and app servers are separated, with the app servers being on a private subnet. Since we already explained the components in our other examples, there is only a little to add:



* The key security control here is creating a Network Security Group to restrict access to the app servers from ONLY the web servers, and only to the specific port and protocol required. 
* The NSG should be applied to each instance, not the subnets, to prevent a "flat network" and peer traffic that could be used in an attack. 
* The app servers can connect to your datacenter, and that is where you would want to route all Internet traffic. That also means you get just as much control over Internet traffic as if the virtual machines were in your own data center.
...
* In this example there is a private subnet in AWS, without either Internet access *or* a connection to the enterprise data center. Images will be created in either another account or a VPC, and nothing will be manually logged into in this architecture.
* When an analytics job is triggered, a server in the data center takes the data and sends it to Amazon S3, their object storage service, using the command line tools or custom code. This is an encrypted connection by default, but you could also encrypt the data using the AWS Key Management Service (or any encryption tool you want). We have clients using both options.
* The S3 bucket in AWS is tightly restricted to either only the IP address of the sending server, or a set of AWS IAM credentials. Or both. AWS maintains S3 security, so you don't worry about network attacks, you merely enable access. It isn't like a public FTP server, if you lock it down (which is easy to do) it isn't visible except from authorized sources.
* A service called AWS *Lambda* monitors the S3 bucket. Lambda is a container for event-driven code running inside Amazon that can trigger based on internal things, like a new file appearing in an S3 bucket. You only pay for Lambda when your code is executing, so there is no cost to have it sitting there.
...
* When the analysis instances launch, the Lambda code also sends them the location of the data within S3 to analyze. The instances connect to S3 through something known as a *VPC Endpoint*, which is totally different from an Azure service endpoint. A VPC endpoint allows instances in a totally private subnet to talk to S3 without needing Internet access (which was required until recently). As of this writing only S3 has a VPC endpoint, but we know additional Amazon services, like their Simple Queue Service, are working on it (we suspect, AWS hasn't confirmed exactly which services are next on the list).
* The instances boot, grab the data, then do their work. When they are done, they go through the S3 VPC Endpoint and drop the results into a second S3 bucket.
...
* The instance can then trigger another Lambda function to send a notification back to the data center that the job is complete, and code in the data center can grab the results. There are actually a few ways to do this, for example the results could go into a database instead.
* When everything is complete, Lambda moves the original data into *Glacier*, Amazon's super-cheap long term archiving storage. In this case, it is, of course, encrypted. (Being network focused research, we are skipping most of the encryption options for this architecture, but those aren't overly difficult).

Think about what we've done - the analysis servers have 0 Internet access, spin up only when needed, and can only read in new data, and write out results. They also terminate when they are finished, so there is no persistent data sitting on a server or in memory. All Internet-facing components are native Amazon services, so we don't need to maintain the network security. Everything is extremely cost effective, even for very large data sets, since we only run processing when we need it, and the big data sets are always stored in the cheapest option possible, and are moved around to minimize overall storage costs. The system is event driven, so if you load 5 jobs at once it runs all 5 at the same time, and if there aren't any jobs, the components sit there in the absolute most cost-effective state.

This example does skip some options that would help resiliency in exchange for better network security. For example, we would normally recommend using Simple Queue Service to manage the jobs (Lambda would send them over) since that will handle situations like an instance failing part way through processing. But hey, this is security research, so why not?

## Conclusion

This research isn't the tip of the iceberg; it's more like the first itty bitty little ice crystal on top of an iceberg that stretches to the depths of the deepest ocean trench. But if you only remember the following principles, you will be just fine as you dig into securing your own cloud and hybrid deployments:
...
* Everything is managed by software, providing tremendous flexibility, and allowing you to manage network security using the exact same tools that development and operations use to manage their pieces of the puzzle.
* You can achieve tremendous security through architecture. Virtual networks (and multiple cloud accounts) support incredible levels of compartmentalization, since every project can have it's own, dedicated network (or networks).
* Security groups enhance that by providing the granularity of a host firewall, without the risks of relying on the operating system. They then provide better manageability than even most network firewalls.
* Platform as a Service and cloud-provider-specific services open up entirely new architectural options. Don't think you need to build things the way you always have. Actually, if you find yourself doing that, you should probably rethink your decision to use the cloud.

Don't be intimidated by cloud computing, but don't think you can, or should, implement network security the way you always have. Your skills and experiences are still important, and provide the base to build upon as you learn all the new options available within the cloud.
